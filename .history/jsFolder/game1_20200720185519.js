...
function updateLasers(dt, $container) {
  ...
  for (let i = 0; i < lasers.length; i++) {
    ...
    const r1 = laser.$element1.getBoundingClientRect();
...
function createEnemy($container, x, y) {
  ...
  // $element3.src = "/jsFolder/pictures/MathApics/invaders_png/red.png";
  $element1.className = "enemy";
  // $element2.className = "enemy";
  // $element3.className = "enemy";
...
function updateEnemies(dt, $container) {
  // rotates enemy around in circular function
  ...
  for (let i = 0; i < enemies.length; i++) {
    ...
    const x = enemy.x + dx;
    const y = enemy.y + dy;
    setPosition(enemy.$element1, x, y);
    enemy.cooldown -= dt;
    if (enemy.cooldown <= 0) {
